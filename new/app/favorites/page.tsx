...
export default function FavoritesPage() {
  ...
  const fetchFavorites = async () => {
    try {
      // Mock favorites data - in real app, fetch from API
      const response = await fetch('/api/listings?limit=20')
      const data = await response.json()
      
      // Simulate some favorites (first 6 listings)
      const mockFavorites = (data.listings || []).slice(0, 6).map((listing: any) => ({
        ...listing,
        favoriteId: `fav_${listing.id}`,
        favoritedAt: new Date(Date.now() - Math.random() * 1000 * 60 * 60 * 24 * 7) // Random time in last week
      }))
  ...
  const filterAndSortFavorites = () => {
    ...
    if (searchQuery) {
      filtered = filtered.filter((favorite: any) =>
  ...
  const handleRemoveFavorite = async (favoriteId: string) => {
    try {
      // Mock removing favorite - in real app, call API
      setFavorites(favorites.filter((fav: any) => fav.favoriteId !== favoriteId))
    } catch (error) {
  ...
  const handleClearAllFavorites = async () => {
    ...
    try {
      // Mock clearing all favorites - in real app, call API
      setFavorites([])
    } catch (error) {
